...
templates.env.globals['DEV_MODE'] = os.getenv('DEV_MODE') == '1'
router = APIRouter()
...
def view_items(
    request: Request,
    db: DbSession,
    search_form: Annotated[ItemSearchForm, Depends(ItemSearchForm.as_query)],
    show_limit: int = 20,
    skip: int = 0,
    ...
    search_values = {
        k: v
        for k, v in search_data.model_dump(exclude_none=True).items()
...
def submit_add_item_form(
    form: Annotated[ItemCreateForm, Depends(ItemCreateForm.as_form)],
    db: DbSession,
) -> RedirectResponse:
...
def submit_edit_item(
    item_id: int,
    update_form: Annotated[ItemUpdateForm, Depends(ItemUpdateForm.as_form)],
    db: DbSession,
) -> RedirectResponse:
...
def update_submission_summary_field(
    db: DbSession,
    payload: SubmissionTableUpdatePayload,
) -> JSONResponse:
    ...
    except SubmissionNumberConflictError:
        return JSONResponse(
            status_code=409,
            content={
                'success': False,
...
def submit_add_submission_form(
    request: Request,
    db: DbSession,
    submission_number: Annotated[int, Form()],
    submission_company: Annotated[str, Form()],
    submission_date: Annotated[str, Form()],
    item_ids: Annotated[list[int], Form()],
) -> Response:
    parsed_date = None if submission_date == '' else date.fromisoformat(submission_date)
...
@router.post('/grading_record_edit/{grading_record_id}', response_class=RedirectResponse)
def submit_edit_grading_record(
    grading_record_id: int,
    update_form: Annotated[GradingRecordUpdateForm, Depends(GradingRecordUpdateForm.as_form)],
    db: DbSession,
) -> RedirectResponse:
